...
<body>
    <form action="{{url('login/update').'/'.$data->user_id}}" method="post" enctype="multipart/form-data">
        @csrf
        <table user_id="{{$data->user_id}}">
            <tr>
                <td>用户名</td>
                <td><input type="text" name="user_name" value="{{$data->user_name}}"><span class="span_name">*</span></td>
            </tr>
            <tr>
                <td>密码</td>
                <td><input type="password" name="user_pwd" value="{{$data->user_pwd}}"><span class="span_pwd">*</span></td>
            </tr>
            <tr>
                <td>手机号</td>
                <td><input type="text" name="user_tel" value="{{$data->user_tel}}"><span class="span_tel">*</span></td>
            </tr>
            <tr>
                <td>邮箱</td>
                <td><input type="text" name="user_text" value="{{$data->user_text}}"><span class="span_text">*</span></td>
            </tr>
            <tr>
                <td>用户头像</td>
                <td><img src="{{env('PICTURE_URL')}}{{$data->user_img}}" height="100px"><input type="file" name="user_img"></td>
            </tr>
            <tr>
                <td></td>
                <td><button class="but">修改</button></td>
            </tr>
        </table>
    </form>
</body>
...
<script>
    $(function(){
        $(document).on('blur','[name=user_name]',function(){    
            ...
            $.ajax({
                type:'post',
                url:"{{url('login/user_sole')}}/"+user_id,
